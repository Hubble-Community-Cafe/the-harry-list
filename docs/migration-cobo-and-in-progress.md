...
1. A new **`IN_PROGRESS`** reservation status, so staff can mark a request as picked up. It is internal only, so the customer is never emailed about it.
...
Deploy the new backend and both frontends. Run the SQL first; after that, deployment order does not matter, as the new fields are optional in the API and older frontends degrade  gracefully (they fall back to a default badge for an unknown status).
